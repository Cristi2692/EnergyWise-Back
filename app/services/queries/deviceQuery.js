...
deviceQuery.consumptionDelete = async(id) =>{
    let consumDelete;
    try{
        consumDelete = await Devices_consumption.destroy({where: {idDevice:id}});
        return true
    }catch(err){
        throw new Error(err)
    }
...
deviceQuery.deleteDevByIduser = async(id, activo) =>{  // eliminar dispositivo por el id de usuario
    let deleteDiv;
    try{
